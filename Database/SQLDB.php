 ...
 * @todo https://www.w3schools.com/php/func_mysqli_options.asp dice que la opcion MYSQLI_OPT_SSL_VERIFY_SERVER_CERT es sólo para mysqlnd. Apañar el código y poner nota en manual php
 * @see  
 * MySQL Info
 ...
 * @param boolean   $VerifySSL              Whether to verify server certificate or not.
        ...
        $Port = NULL, $Socket = NULL)
{
    ...
    {
        $InData['Options']['VerifySSL'] = $VerifySSL;
    }
 ...
 *                                                                          authentication
 *                                          'VerifySSL'                 Flag MYSQLI_OPT_SSL_VERIFY_SERVER_CERT, whether to verify server certificate or not.
                            ...
                            foreach ($InData[$Key] as $OptionKey => $OptionValue)
                            {
                                switch ($OptionKey)
                                {
                                    ...
                                    case 'UseSSL':
                                        ...
                                        {
                                            if (isset($InData['Options']['DoNotVerifyServerCert']))
                                            {
                                                if ($InData['Options']['DoNotVerifyServerCert'] === TRUE)
                                                {
                                                    //Both UseSSL and DoNotVerifyServerCert cannot be set to true
                                                    $AllGood = FALSE;
                                                    $InvalidParameters .= 'Both UseSSL and DoNotVerifyServerCert cannot be set to TRUE, ';
                                                }
                                            }
                                        }
                                        break;
                                    case 'DoNotVerifyServerCert':
                                        ...
                                        {
                                            if (isset($InData['Options']['UseSSL']))
                                            {
                                                if ($InData['Options']['UseSSL'] === TRUE)
                                                {
                                                    //Both UseSSL and DoNotVerifyServerCert cannot be set to true
                                                    $AllGood = FALSE;
                                                    $InvalidParameters .= 'Both UseSSL and DoNotVerifyServerCert cannot be set to TRUE, ';
                                    ...
                                    case 'Socket':
                                        ...
                                        if (($InData['ServerName'] !== 'localhost')||(is_writable($OptionValue) === FALSE))
                                        {
                                            $AllGood = FALSE;
                                            $InvalidParameters .= 'Socket: '.$OptionValue.'not writeable or used without specifying "localhost" as server name';
                                        }
                                    default:
...
*/
    ...
    $GLOBALS['DB'][$FreeCell]['System'] = $InData['System'];
    $GLOBALS['DB'][$FreeCell]['ServerName'] = $InData['ServerName'];
    $GLOBALS['DB'][$FreeCell]['Database'] = $InData['Database'];
    $GLOBALS['DB'][$FreeCell]['DBUser'] = $InData['DBUser'];
    $GLOBALS['DB'][$FreeCell]['DBPassword'] = $InData['DBPassword'];
    if (isset($InData['Options']))
    {
        $GLOBALS['DB'][$FreeCell]['Options'] = $InData['Options'];
    }
...
function MySQLRealConnect($InData, &$OutData, $ConnectionLink)
{
    ...
    //Connect
    $ConnectResult = mysqli_real_connect($ConnectionLink, $InData['ServerName'], $InData['DBUser'], $InData['DBPassword'], $InData['Database'] ,
            $ThePort, $TheSocket, $ConnectionFlags);
